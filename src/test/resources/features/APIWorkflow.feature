Feature: API workflow test
  ...
  Scenario: Getting the created employee
    ...
    |Aram         |Lango       |ms             |Male      |2010-01-14  |confirmed |QA Engineer  |
  ...
  Scenario: API test case for creating the employee using highly dynamic body
    Given a request is prepared for creating an employee with dynamic data "Aram", "Lango", "ms", "M", "2010-01-14", "confirmed", "QA Engineer"
    When a POST call is made to create an employee
    Then the status code for creating an employee is 201
    And the response body contains key "Message" and value "Employee Created"
    And the employee id "Employee.employee_id" is stored as global to be used for other request
  ...
  @apidynamic
  Scenario: API test case for updating the created employee using highly dynamic body
    Given a request is prepared for updating the created employee with dynamic data "empId", "Yuza", "Wali", "ms", "F", "2023-01-15", "Maternity-leave", "Manager"
